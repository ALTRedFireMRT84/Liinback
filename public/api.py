...
class Invidious:
    ...
    def buildXML(self, json_data, lang):
        viewCountLabels = {
            'en': 'views',
            'es': 'visualizaciones',
            'fr': 'vues',
        ...
        for item in json_data:
            if 'videoId' not in item:
                continue
            xml_string += '<entry>'
            xml_string += f'<id>http://liinback.pythonanywhere.com/api/videos/' + self.escape_xml(item['videoId']) + '</id>'
            ...
            xml_string += f'<link rel=\'http://liinback.pythonanywhere.com/api/videos/' + self.escape_xml(item['videoId']) + '/related\'/>'
            xml_string += '<author>'
            xml_string += '<name>' + self.escape_xml(item['author']) + '</name>'
            xml_string += f'<uri>http://liinback.pythonanywhere.com/api/channels/' + self.escape_xml(item.get('channel_id', '')) + '</uri>'
            xml_string += '</author>'
    ...
    def buildPlaylistXML(self, json_data, noDateString):
        xml_string = '<?xml version=\'1.0\' encoding=\'UTF-8\'?>'
        xml_string += '<feed xmlns:openSearch=\'http://a9.com/-/spec/opensearch/1.1/\' xmlns:media=\'http://search.yahoo.com/mrss/\' xmlns:yt=\'http://www.youtube.com/xml/schemas/2015\'>'
        xml_string += '<title type=\'text\'>Videos</title>'
        xml_string += '<generator ver=\'1.0\' uri=\'http://kamil.cc/\'>Liinback data API</generator>'
    ...
    def buildM3U(json_data):
        ...
        for item in json_data:
            ...
            get_video_url = f"http://liinback.pythonanywhere.com/api/v2/videos/{video_id}"
            m3u_content += f"#EXTINF:-1,{title}\n{thumbnail_url}\n{get_video_url}\n"
        return m3u_content

    @staticmethod
    def buildV2M3U(json_data):
        m3u_content = "#EXTM3U\n"
        for item in json_data:
            if 'videoId' not in item:
                continue
    ...
    @staticmethod
    def buildPlaylistM3U(json_data):
        m3u_content = "#EXTM3U\n"
        for item in json_data:
            if 'videoId' not in item:
                continue
    ...
    def search(self, query):
        ...
        xml_data = self.buildM3U(json_data)
    ...
    def trends(self, type_param=None):
        ...
        if response.status_code == 200:
            json_data = response.json()
            xml_data = self.buildV2M3U(json_data)
    ...
    def user_uploads(self, lang, channel_id=None):
        ...
        if response.status_code == 200:
            ...
            xml_data = self.buildXML(videos, lang)
            return Response(xml_data, mimetype='text/atom+xml')
        else:
            error_message = {"error": "Invalid response format"}
            return jsonify(error_message), 500
            
    def user_uploads_V2(self, channel_id=None):
        channels_endpoint = "channels"
        response = requests.get(f"http://vid.puffyan.us/api/v1/{channels_endpoint}/{channel_id}/videos")
        if response.status_code == 200:
            json_data = response.json()
            videos = json_data.get("videos", [])
            xml_data = self.buildM3U(videos)
            return Response(xml_data, mimetype='text/plain')
        else:
            error_message = {"error": "Invalid response format"}
            return jsonify(error_message), 500

    def playlists(self, noDateString, playlist_id=None):
        playlists_endpoint = "playlists"
        response = requests.get(f"http://vid.puffyan.us/api/v1/{playlists_endpoint}/{playlist_id}")
        if response.status_code == 200:
            json_data = response.json()
            videos = json_data.get("videos", [])
            xml_data = self.buildPlaylistXML(videos, noDateString)
            return Response(xml_data, mimetype='text/atom+xml')
        else:
            error_message = {"error": "Invalid response format"}
            return jsonify(error_message), 500
            
    def playlistV2(self, playlist_id=None):
        playlists_endpoint = "playlists"
        response = requests.get(f"http://vid.puffyan.us/api/v1/{playlists_endpoint}/{playlist_id}")
        if response.status_code == 200:
            json_data = response.json()
            videos = json_data.get("videos", [])
            xml_data = self.buildPlaylistM3U(videos)
            return Response(xml_data, mimetype='text/plain')
        else:
            error_message = {"error": "Invalid response format"}
            return jsonify(error_message), 500            
...
class InnerTube:
    def playlist(self, lang, playlist_id, oauth_token=None):
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildPlaylistXML(data)
            return Response(xml_data, mimetype='text/atom+xml')
        else:
            return jsonify({"error": "Failed to fetch playlist data"}), 500
            
    def buildPlaylistXML(self, json_data):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        xml_string += '<feed xmlns:openSearch="http://a9.com/-/spec/opensearch/1.1/" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://www.youtube.com/xml/schemas/2015">'
                        ...
                        if "playlistVideoListRenderer" in video_item:
                            for video in video_item["playlistVideoListRenderer"].get("contents", []):
                                if "playlistVideoRenderer" in video:
                                    ...
                                    xml_string += '<entry>'
                                    xml_string += f'<id>http://liinback.pythonanywhere.com/api/videos/{videoId}</id>'
                                    xml_string += f'<published>{self.escape_xml(publishedText)}</published>'
                                    xml_string += f'<title type="text">{self.escape_xml(title)}</title>'
                                    xml_string += f'<link rel="http://liinback.pythonanywhere.com/api/videos/{videoId}/related"/>'
    ...
    def buildPlaylistsXML(self, json_data, oauth_token):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        xml_string += '<feed xmlns:openSearch=\'http://a9.com/-/spec/opensearch/1.1/\' xmlns:media=\'http://search.yahoo.com/mrss/\' xmlns:yt=\'http://www.youtube.com/xml/schemas/2015\'>'
        xml_string += '<title type="text">Playlist</title>'
        xml_string += '<generator ver="1.0" uri="http://kamil.cc/">Liinback data API</generator>'
                ...
                for item in item_section:
                    ...
                    for video in horizontal_list:
                        ...
                        if title_metadata and browse_id:
                            ...
                            xml_string += f"<link>http://liinback.pythonanywhere.com/api/_playlists?id={self.escape_xml(browse_id)}&access_token={oauth_token}'</link>"
                            if thumbnail_url:
                                xml_string += '<media:group>'
        ...
        xml_string += '</feed>'
        return xml_string    
    def playlists(self, oauth_token, lang):
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildPlaylistsXML(data, oauth_token)
            return Response(xml_data, mimetype='text/atom+xml')

        return Response('Failed to fetch playlists', status=500)

    def buildWatchHistory2XML(self, json_data, lang):
        viewCountLabels = {
            'en': 'views',
            'es': 'visualizaciones',
            'fr': 'vues',
    ...
    def buildWatchHistoryXML(self, json_data, lang, oauth_token):
        viewCountLabels = {
            'en': 'views',
            'es': 'visualizaciones',
            'fr': 'vues',
    ...
    def subscriptions(self, oauth_token):
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildSubscriptions(oauth_token, data)
            return Response(xml_data, mimetype='text/atom+xml')

    def buildExplorerXML(self, json_data):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        ...
        xml_string += f'<link rel="next" type="application/atom+xml" href="http://liinback.pythonanywhere.com/api/videos"/>'
                        ...
                        if 'shelfRenderer' in item:
                            ...
                            for video_item in shelf_content:
                                ...
                                if video_renderer:
                                    ...
                                    xml_string += '<entry>'
                                    xml_string += f'<id>http://liinback.pythonanywhere.com/api/videos/{video_id}</id>'
                                    xml_string += f'<published>{self.escape_xml(published_text)}</published>'
                                    xml_string += f'<title type="text">{self.escape_xml(title)}</title>'
                                    xml_string += f'<link rel="http://liinback.pythonanywhere.com/api/videos/{video_id}/related"/>'
                                    xml_string += f'<author><name>{self.escape_xml(author_name)}</name>'
                                    xml_string += f'<uri>http://liinback.pythonanywhere.com/channels/{author_id}/uploads</uri></author>'
                                    xml_string += '<media:group>'
    ...
    def buildSubscriptions(self, json_data, oauth_token):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        xml_string += '<feed xmlns:openSearch="http://a9.com/-/spec/opensearch/1.1/" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://www.youtube.com/xml/schemas/2015">'
        xml_string += f'<link>f"http://liinback.pythonanywhere.com/api/subscriptions?access_token={oauth_token}"</link>'
        xml_string += '<title type="text">Subscriptions</title>'
    ...
    @staticmethod
    def buildCategoryThreeXML(json_data):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        xml_string += '<feed xmlns:openSearch="http://a9.com/-/spec/opensearch/1.1/" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://www.youtube.com/xml/schemas/2015">'
        xml_string += '<title type="text">Videos</title>'
        xml_string += '<generator ver="1.0" uri="http://kamil.cc/">Liinback data API</generator>'
    ...
    @staticmethod
    def buildCategoryThreeM3U(json_data):
        m3u_content = "#EXTM3U\n"
        for item in json_data.get("contents", {}).get("twoColumnBrowseResultsRenderer", {}).get("tabs", []):
            tab = item.get("tabRenderer", {})
            section_list = tab.get("content", {}).get("richGridRenderer", {}).get("contents", [])
            ...
            for section in section_list:
                if "richSectionRenderer" in section:
                    ...
                    title = video_data.get("title", {}).get("runs", [{}])[0].get("text", "")
                    thumbnail_url = f"http://i.ytimg.com/vi/{video_id}/mqdefault.jpg"
                    get_video = f"http://liinback.pythonanywhere.com/api/v2/videos/get/{video_id}"
                    m3u_content += f"#EXTINF:-1,{title}\n{thumbnail_url}\n{get_video}\n"
        return m3u_content

    @staticmethod
    def buildRiverXML(json_data):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        xml_string += '<feed xmlns:openSearch="http://a9.com/-/spec/opensearch/1.1/" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://www.youtube.com/xml/schemas/2015">'
        xml_string += '<title type="text">Videos</title>'
        xml_string += '<generator ver="1.0" uri="http://kamil.cc/">Liinback data API</generator>'
    ...
    def buildCategoryXML(self, json_data):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        xml_string += '<feed xmlns:openSearch="http://a9.com/-/spec/opensearch/1.1/" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://www.youtube.com/xml/schemas/2015">'
        xml_string += '<title type="text">Videos</title>'
        xml_string += '<generator ver="1.0" uri="http://kamil.cc/">Liinback data API</generator>'
        ...
        return xml_string
    @staticmethod    
    def buildCategoryM3U(json_data):
        m3u_content = "#EXTM3U\n"
        for item in json_data.get("contents", {}).get("twoColumnBrowseResultsRenderer", {}).get("tabs", []):
            tab = item.get("tabRenderer", {})
            section_list = tab.get("content", {}).get("sectionListRenderer", {}).get("contents", [])
                        ...
                        if "shelfRenderer" in video_item:
                            ...
                            for horizontal_item in horizontal_list:
                                if "videoRenderer" in horizontal_item:
                                    ...
                                    title = video_data.get("title", {}).get("runs", [{}])[0].get("text", "")
                                    thumbnail_url = f"http://i.ytimg.com/vi/{video_id}/mqdefault.jpg"
                                    get_video = f"http://liinback.pythonanywhere.com/api/v2/videos/{video_id}"
                                    m3u_content += f"#EXTINF:-1,{title}\n{thumbnail_url}\n{get_video}\n"
        return m3u_content        
        
    def buildCategoryTwoXML(self, json_data):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        xml_string += '<feed xmlns:openSearch="http://a9.com/-/spec/opensearch/1.1/" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://www.youtube.com/xml/schemas/2015">'
        xml_string += '<title type="text">Videos</title>'
        xml_string += '<generator ver="1.0" uri="http://kamil.cc/">Liinback data API</generator>'
    ...
    @staticmethod
    def buildCategoryTwoM3U(json_data):
        m3u_content = "#EXTM3U\n"
        for item in json_data.get("contents", {}).get("twoColumnBrowseResultsRenderer", {}).get("tabs", []):
            tab = item.get("tabRenderer", {})
            section_list = tab.get("content", {}).get("sectionListRenderer", {}).get("contents", [])
    ...
    def buildUploadsXML(self, json_data):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        xml_string += '<feed xmlns:openSearch="http://a9.com/-/spec/opensearch/1.1/" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://www.youtube.com/xml/schemas/2015">'
        xml_string += '<title type="text">Videos</title>'
        xml_string += '<generator ver="1.0" uri="http://kamil.cc/">Liinback data API</generator>'
    ...
    def buildSearchXML(self, json_data):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        xml_string += '<feed xmlns:openSearch="http://a9.com/-/spec/opensearch/1.1/" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://www.youtube.com/xml/schemas/2015">'
        xml_string += '<title type="text">Videos</title>'
        xml_string += '<generator ver="1.0" uri="http://kamil.cc/">Liinback data API</generator>'
    ...
    def buildSearchM3U(self, json_data):
        m3u_content = "#EXTM3U\n"
        for section in json_data.get("contents", {}).get("twoColumnSearchResultsRenderer", {}).get("primaryContents", {}).get("sectionListRenderer", {}).get("contents", []):
            if "itemSectionRenderer" in section:
                for video_item in section["itemSectionRenderer"].get("contents", []):
                    if "videoRenderer" in video_item:
                        ...
                        getVideo = f"http://liinback.pythonanywhere.com/api/v2/videos/get/{videoId}"
                        m3u_content += f"#EXTINF:-1,{title}\n{thumbnailUrl}\n{getVideo}\n"
        return m3u_content

    def buildVideoInfoXML(self, json_data, lang):
        viewCountLabels = {
            'en': 'views',
            'es': 'visualizaciones',
            'fr': 'vues',
    ...
    def watchHistory(self, lang, oauth_token):
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildWatchHistoryXML(data, lang, oauth_token)
            return Response(xml_data, mimetype='text/atom+xml')
            
    def watchHistory2(self, lang, oauth_token):
        url = f'https://www.youtube.com/youtubei/v1/browse?browseId=FEhistory&key=AIzaS=yAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8&access_token={oauth_token}'
        headers = {
            'Content-Type': 'application/json',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildWatchHistory2XML(data, lang)
            return Response(xml_data, mimetype='text/atom+xml')

    def river(self, oauth_token, lang):
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildRiverXML(data)
            return Response(xml_data, mimetype='text/atom+xml')

    def watchLater(self, lang, oauth_token):
    ...
    def likedVideos(self, oauth_token, lang):
    ...
    def player(self, video_id, lang, oauth_token):
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildVideoInfoXML(data, lang)
            return Response(xml_data, mimetype='text/atom+xml')

    def searchV3(self, query):
        ...
        if response.status_code == 200:
            data = response.json()
            m3u_data = self.buildSearchM3U(data)
    ...
    def search(self, query, lang):
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildSearchXML(data)
            return Response(xml_data, mimetype='text/atom+xml')
            
    def userUploads(self, lang, channelId):
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildUploadsXML(data)
            return Response(xml_data, mimetype='text/atom+xml')

    def favorites(self, lang, oauth_token):
        url = f'https://www.youtube.com/youtubei/v1/browse?browseId=FElibrary&key=AIzaS=yAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8&access_token={oauth_token}'
        headers = {
            'Content-Type': 'application/json',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
        ...
        if response.status_code == 200:
            ...
            for item in data.get("contents", {}).get("twoColumnBrowseResultsRenderer", {}).get("tabs", []):
                ...
                if favoritesId:
                    break
            if favoritesId:
                xml_data = self.buildPlaylistXML(favoritesId)
                return Response(xml_data, mimetype='text/atom+xml')
            else:
                return {"error": "Favorites playlist not found."}, 404
            
    def category(self, type, params, lang, oauth_token=None):
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildCategoryXML(data)
            return Response(xml_data, mimetype='text/atom+xml')
            
    def categoryV2(self, type, params):
        url = f'https://www.youtube.com/youtubei/v1/browse?browseId=FE{type}&key=AIzaS=yAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8'
        if params:
            url += f'&params={params}'
        headers = {
        ...
        if response.status_code == 200:
            data = response.json()
            m3u_data = self.buildCategoryM3U(data)
            return Response(m3u_data, mimetype='text/plain')
            
    def categoryTwo(self, channelId, lang, oauth_token=None):
        url = f'https://www.youtube.com/youtubei/v1/browse?browseId={channelId}&key=AIzaS=yAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8'
        if oauth_token:
            url += f'&access_token={oauth_token}'
        headers = {
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildCategoryTwoXML(data)
            return Response(xml_data, mimetype='text/atom+xml')
            
    def categoryTwoV2(self, channelId):
        url = f'https://www.youtube.com/youtubei/v1/browse?browseId={channelId}&key=AIzaS=yAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8'
        headers = {
            'Content-Type': 'application/json',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildCategoryTwoM3U(data)
            return Response(xml_data, mimetype='text/plain')
            
    def categoryThree(self, channelId, lang, oauth_token=None):
        url = f'https://www.youtube.com/youtubei/v1/browse?browseId={channelId}&key=AIzaS=yAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8'
        if oauth_token:
            url += f'&access_token={oauth_token}'
        headers = {
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildCategoryThreeXML(data)
            return Response(xml_data, mimetype='text/atom+xml')
            
    def categoryThreeV2(self, channelId):
        url = f'https://www.youtube.com/youtubei/v1/browse?browseId={channelId}&key=AIzaS=yAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8'
        headers = {
            'Content-Type': 'application/json',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildCategoryThreeM3U(data)
            return Response(xml_data, mimetype='text/plain')

    def explore(self, lang, oauth_token=None):
        ...
        if response.status_code == 200:
            data = response.json()
            xml_data = self.buildExplorerXML(data)
            return Response(xml_data, mimetype='text/atom+xml')
            
class DataAPI:
    DATA_API_KEY = 'AIzaSyDuziy6qTSaKhs0w4SKpLNWivKFNoWjQQA'    
    BASE_URL = "https://www.youtube.com/youtube/v3"
    ...
    def userUploads(self, lang, channelId):
        ...
        xml_data = self.buildUploadsXML(videos)
        return Response(xml_data, mimetype='text/atom+xml')
    ...
    def playlist(self, playlistId, oauthToken=None):
        if oauthToken:
        ...
        xml_data = self.buildPlaylistXML(videos)
        return Response(xml_data, mimetype='text/atom+xml')

    def getAllVideosFromPlaylistWithOAuth(self, playlistId, oauthToken):
        videos = []
        url = f"https://www.youtube.com/youtube/v3/playlistItems?part=snippet,contentDetails&maxResults=50&playlistId={playlistId}&access_token={oauthToken}"
        ...
        while url:
            ...
            if response.status_code == 200:
                ...
                url = data.get("nextPageToken")
                if url:
                    url = f"https://www.youtube.com/youtube/v3/playlistItems?part=snippet,contentDetails&maxResults=50&pageToken={url}&playlistId={playlistId}&access_token={oauthToken}"
                else:
                    break
        return videos

    def buildPlaylistXML(self, videos):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        xml_string += '<feed xmlns:openSearch="http://a9.com/-/spec/opensearch/1.1/" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://www.youtube.com/xml/schemas/2015">'
        xml_string += '<title type="text">Playlist</title>'
        xml_string += '<generator ver="1.0" uri="http://kamil.cc/">Liinback data API</generator>'
    ...
    def buildUploadsXML(self, videos):
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>'
        xml_string += '<feed xmlns:openSearch="http://a9.com/-/spec/opensearch/1.1/" xmlns:media="http://search.yahoo.com/mrss/" xmlns:yt="http://www.youtube.com/xml/schemas/2015">'
        xml_string += '<title type="text">Videos</title>'
        xml_string += '<generator ver="1.0" uri="http://kamil.cc/">Liinback data API</generator>'
